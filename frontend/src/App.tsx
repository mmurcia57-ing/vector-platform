...
function CommitmentManagementPanel() {
  const [data, setData] = useState<CommitmentManagement>(); const [filter, setFilter] = useState(''); const [declaration, setDeclaration] = useState(''); const [areaDomainId, setAreaDomainId] = useState('area-platform'); const [dueDate, setDueDate] = useState(''); const [status, setStatus] = useState('OPEN');
  const load = () => { void read<CommitmentManagement>('/api/experience/commitments?asOf=2025-01-01&limit=20').then(setData) }
  useEffect(load, [])
  const create = (event: FormEvent) => { event.preventDefault(); void fetch('/api/experience/commitments', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ commitmentId: `commitment-${Date.now()}`, declaration, accountableAreaDomainId: areaDomainId, dueDate: dueDate || null, executionStatus: status, intendedResult: declaration }) }).then(() => { setDeclaration(''); load() }) }
  const visible = data?.commitments.filter((item) => item.declaration.toLowerCase().includes(filter.toLowerCase())) ?? []
  return <section className="commitments" aria-label="Commitments and Improvements"><h2>Commitments &amp; Improvements</h2><input aria-label="Filter commitments" placeholder="Filter commitments" value={filter} onChange={(event) => setFilter(event.target.value)} /><form onSubmit={create}><input aria-label="Commitment declaration" required placeholder="Commitment declaration" value={declaration} onChange={(event) => setDeclaration(event.target.value)} /><input aria-label="Accountable AreaDomain" required value={areaDomainId} onChange={(event) => setAreaDomainId(event.target.value)} /><input aria-label="Due date" type="date" value={dueDate} onChange={(event) => setDueDate(event.target.value)} /><select aria-label="Execution status" value={status} onChange={(event) => setStatus(event.target.value)}><option>OPEN</option><option>IN_PROGRESS</option><option>COMPLETED</option></select><button type="submit">Create Commitment</button></form><p>Active: {data?.activeCount ?? 0} · In progress: {data?.inProgressCount ?? 0} · Completed: {data?.completedCount ?? 0} · Overdue: {data?.overdueCount ?? 0}</p>{visible.map((item) => <article key={item.commitmentId} onClick={() => setFilter(item.declaration)}><strong>{item.declaration}</strong><span>{item.executionStatus}{item.overdue ? ' · OVERDUE' : ''}</span><small>AreaDomain: {item.accountableAreaDomainId} · Source: {item.sourceReferenceSummary}</small></article>)}</section>
}

function QualityPanel({ quality }: { quality: Quality }) { return <aside className="quality" aria-label="Data quality and limits"><strong>Evidence and limits</strong><span>Panorama Ejecutivo: attention uses available context.</span><span>J02 Change/Deployment context is temporal/contextual; Correlation != Causation.</span><span>Coverage: {quality.sourceCoverage}</span><span>Freshness: {quality.freshness}</span><span>Confidence: {quality.confidenceContext}</span>{quality.uncertainty.map((item) => <span key={item}>Uncertainty: {item}</span>)}{quality.missingContext.map((item) => <span key={item}>Missing context: {item}</span>)}{quality.limitations.map((item) => <span key={item}>Limit: {item}</span>)}<CommitmentManagementPanel /></aside> }
