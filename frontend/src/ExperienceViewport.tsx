...
function NorthStarUniverse({ overview, commitments, period, onPeriodChange, navigate, tr }: { overview: Overview; commitments?: CommitmentView; period: string; onPeriodChange: (period: string) => void; navigate: (next: string, context?: Record<string,string>) => void; tr: (es:string,en:string)=>string }) {
  ...
  const services = overview.services.slice(0, 8);
  const positions = [
    [16,48],[38,27],[53,50],[75,32],[78,69],[37,72],[20,76],[68,78],
  ];
  ...
  return <div className="vx-northstar-shell">
    <header className="vx-ns-top">
      ...
      <span className="vx-ns-quality">{overview.quality.stale ? t.stale : overview.quality.partial ? t.partial : t.available}</span>
    </header>
    ...
    <main className="vx-ns-universe">
      ...
      <svg className="vx-ns-edges" viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true">
        {services.slice(1).map((service,index)=>{const a=positions[0],b=positions[index+1]??positions[0];return <line key={service.serviceId} x1={a[0]} y1={a[1]} x2={b[0]} y2={b[1]} />})}
      </svg>
    ...
    <aside className="vx-ns-inspector">
      ...
      <div className="vx-ns-derived"><b>{t.derived.toUpperCase()}</b><span>{selectedRisk?.condition ?? t.supportedContext}</span></div>
      <div className="vx-ns-limit"><b>{tr("LIMITACIÓN","LIMITATION")}</b><span>{t.contextDisclaimer}</span></div>
      ...
      <div className="vx-ns-commit"><small>{tr("COMPROMISOS ACTIVOS","ACTIVE COMMITMENTS")}</small><strong>{commitments?.activeCount ?? 0}</strong><span>{tr("Ejecución ≠ resultado verificado","Execution ≠ verified outcome")}</span></div>
    </aside>
