...
test("commitment source wiring exposes governed lifecycle handlers without coupling assurance to UI language",()=>{
  for(const token of ["updateCommitmentStatus","renegotiateCommitment","commitmentReliabilityRate","outcomePendingCount"]) assert.match(viewport,new RegExp(token));
  assert.match(viewport,/\/api\/experience\/commitments/);
});
